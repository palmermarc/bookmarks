...
export default function IconRenderer({ icon, className = "w-4 h-4", style }: IconRendererProps) {
  if (!icon) {
    return null;
  }
  ...
  if (icon.startsWith('fa-solid')) {
    ...
    if (solidIcon) {
      return <FontAwesomeIcon icon={solidIcon} className={className} style={style} />;
    }
    ...
    if (HeroIcon) {
      return <HeroIcon className={className} style={style} />;
    }
  }
